      ...
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <h2 id="timeline-heading" className="text-4xl font-bold text-foreground tracking-tight mb-4">
              Your 12-Week Journey to Career Success
            </h2>
          ...
          <div className="flex justify-center mb-8 flex-wrap gap-3">
            {timeline.map((phase, index) => (
              <Button
                ...
                size="sm"
                onClick={() => setActiveWeek(index)}
                className="text-xs md:text-sm rounded-full px-4 py-1 shadow-sm hover:shadow-md transition-all"
              >
                {phase.week}
              </Button>
            ))}
          ...
          <div className="relative mb-16 hidden md:block">
            ...
            <div
              className="absolute top-8 left-0 h-1 bg-primary transition-all duration-500 rounded-full"
              style={{ width: `${(activeWeek / (timeline.length - 1)) * 100}%` }}
            />
            <div className="relative flex justify-between">
              {timeline.map((phase, index) => (
                ...
                >
                  ...
                  <span className="text-xs mt-2 font-medium text-center max-w-[80px] text-muted-foreground">
                    {phase.week}
                  </span>
                </button>
              ))}
          ...
          <Card className="border border-primary/20 shadow-xl rounded-3xl backdrop-blur-sm bg-card/70">
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-foreground leading-tight">
                    {timeline[activeWeek].week}
                  </h3>
                  <p className="text-lg text-primary font-semibold">
                    {timeline[activeWeek].title}
                  </p>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-10">
                <div>
                  ...
                  <ul className="space-y-4">
                    {timeline[activeWeek].milestones.map((milestone, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <ArrowRight className="w-5 h-5 text-secondary flex-shrink-0 mt-1" aria-hidden="true" />
                ...
                <div>
                  ...
                  <ul className="space-y-4">
                    {timeline[activeWeek].deliverables.map((deliverable, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <ArrowRight className="w-5 h-5 text-accent flex-shrink-0 mt-1" aria-hidden="true" />
                        <span className="text-muted-foreground leading-relaxed text-sm md:text-base">
                          {deliverable}
                        </span>
                      </li>
