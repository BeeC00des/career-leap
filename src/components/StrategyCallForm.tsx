...
const formSchema = z.object({
  ...
  university: z.string().min(2, "Please enter your university"),
  careerStatus: z.enum(["final-year-student", "recent-graduate", "career-changer", "other"]),
  goal: z.string().min(10, "Please describe your goal"),
  callLanguage: z.enum(["english", "german"]),
  timeSlot: z.date({
...
const StrategyCallForm = ({ open, onOpenChange }: StrategyCallFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const { data: dbData, error: dbError } = await supabase.from("book").insert({
        ...
        goal: values.goal,
        callLanguage: values.callLanguage,
        timeSlot: values.timeSlot.toISOString(),
      });
      if (dbError) {
        console.log(dbError);
        throw dbError;
      ...
      toast({
        title: "Call confirmed!",
        description: "We'll send a calendar invite shortly.",
      });
    } catch (err: any) {
    ...
    return (
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="text-center py-6">
            <p className="text-muted-foreground mb-4">We'll send you a calendar invite shortly.</p>
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">Book a Free Strategy Call</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Get 1:1 guidance from a CareerLeap coach.
          </DialogDescription>
        </DialogHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="University of Berlin" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Explain what you want to achieve..." className="min-h-[100px]" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
